...
const fileProxy = async (options, file) => {
	...
	const proxyPkg = {
		...
		private: true,
		main: `../${cjsDir}/${file}.js`,
		module: `../${esmDir}/${file}.js`,
	}

	if (typeof typesDir === 'string') {
		proxyPkg.types = `../${typesDir}/${file}.d.ts`
	} else if (await isFile(`${cwd}/${file}.d.ts`)) {
		proxyPkg.types = `../${file}.d.ts`
		// try the esm path in case types are located with each
	} else if (await isFile(`${cwd}/${esmDir}/${file}.d.ts`)) {
		proxyPkg.types = `../${esmDir}/${file}.d.ts`
	}
